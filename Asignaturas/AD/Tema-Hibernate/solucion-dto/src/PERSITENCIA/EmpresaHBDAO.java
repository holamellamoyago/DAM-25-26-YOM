...
public class EmpresaHBDAO {
        ...
        try (Session sesion = HibernateUtil.getSessionFactory().openSession()) {
            ...
            if (d.getFunciones().contains(funcion)) {
                throw new RuntimeException(
                        "A funci�n '" + funcion + "' xa est� asignada ao departamento " + numDepartamento + ".");
            }
            // Engadir a funci�n
     ...
     * 
     * Si solo actualizamos un lado, la relaci�n en memoria queda inconsistente y
     * Hibernate puede:
     * - No persistir correctamente la relaci�n en la base de datos.
     * - Dar comportamientos inesperados al navegar por la colecci�n del
     * departamento.
     * 
     * Por eso, para mantener la **coherencia entre objetos Java y la base de
     * datos**, siempre se deben sincronizar ambos lados de la asociaci�n.
     */
    public static int crearEmpregadoConDepartamentoDAO(
        ...
        try (Session sesion = HibernateUtil.getSessionFactory().openSession()) {
            ...
            // Es transistorio , no est� asociado con la sessin actual si lo traigo en una
            // funcion
            ...
            /*
             * Este es el �nico cambio que Hibernate necesita para actualizar la base de
             * datos, porque:
             * El lado MANY (Empregado) contiene la clave for�nea.Hibernate solo mira ese
             * lado para generar el UPDATE
             * Por tanto: Con esta l�nea, la relaci�n queda correctamente persistida en la
             * base de datos.
             */
             ...
             */
            departamento.getEmpregados().add(empregado); // Lado "uno" ? a�adimos el empleado a la colecci�n del
                                                         // departamento

            // 4. Persistencia
            /*
             * empregado es transitaorio y por eso lo tenemos que persisteir
             * Departameto lo hemos obtenido con get y ya est� en el contexto de
             * persistencia
    ...
    public static int gardarOuActualizarVehiculoDAO(String nss, Vehiculo v) throws HibernateException {
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            if (vehEmp == null) {
                ...
                // el lado propietario (Vehiculo), porque es el �nico que escribe la relaci�n en
                // la base de datos.
                ...
                // luego se utiliza si que har�a falta para que tenga constancia
                session.save(v);
            } else {
                // Actualizaci�n
    ...
    public static List<EmpregadoProxectoDTO> listarEmpleadosProxectoDAO() {
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            return session.createQuery(hql, EmpregadoProxectoDTO.class).list(); // getResultList() es JPA e list() m�is
                                                                                // com�n en Hibernate puro
        }
    }
    ...
    public static boolean borrarProxectoPorNome(String nomeProxecto) {
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
             ...
             * ao borrar un proxecto Hibernate non eliminar�a automaticamente as relaci�ns
    ...
    public static int insertarDepartamentoCompleto(Departamento dpto, Set<String> lugares,
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            tx = session.beginTransaction();
            /*
             * O departamento � o propietario l�xico da relaci�n co director, pero en
             * Hibernate sempre se debe actualizar o lado que cont�n
             * a clave for�nea (FK). Neste caso, a FK NSSDirector est� na t�boa
            ...
            if (lugares != null) {
                for (String l : lugares) {
                    ...
                    novoLugar.setDepartamento(dpto); // �nico lado que Hibernate usa para actualizar a base de datos � o
                                                     // lado propietario, � dicir:� o lado que cont�n a FK
                                                     // Num_departamento.
                    dpto.getLugares().add(novoLugar); // NON � necesario para insertar en la bd BD, pero SI para manter
                                                      // o modelo en memoria consistente.
                    // Se non o fas, dpto.getLugares() non mostrar� o novo lugar ata recargar desde
                    // a BD.
                }
            }
